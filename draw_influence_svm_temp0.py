...
# minmax_p = np.load('/scratch0/GoGradients/code/exp/ploting/exp1/max.npy')
# minmax_idx = minmax_p.argsort()[::-1][:30]
# minmin_p = np.load('/scratch0/GoGradients/code/exp/ploting/exp1/min.npy')
# minmin_idx = minmin_p.argsort()[::-1][:30] # choosing max
# minmin_idx = minmin_p.argsort()[::-1][:30] # choosing min
# minmax_res_dir = 'LR_influence/adult_minmax'
# minmin_res_dir = 'LR_influence/adult_minmin'
# minmax_res_dir = 'svm_influence/adult'
# minmin_res_dir = 'svm_influence/adult'
...
random_dir = 'svm_influence/adult'
res_dir = 'svm_influence/adult'
sh_dir = 'svm_influence'
figure_title = "{} vs {}\n(on Adult, influence computed with Soft-Hinge SVM)"

out_dir = 'svm_influence/adult_svminf'
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

influence_list, hinge_list, gt_list = [], [], []

def load_data(gt_datadict, hingeinf_datadict, influence_list, hinge_list, gt_list, idx):
    total_inf = hingeinf_datadict['total_inf']
    hinge = hinge_losses[idx]
    influence_list.append(total_inf)
    hinge_list.append(hinge)

    gt = np.sum(np.abs(gt_datadict['ground_truth']))
    gt_list.append(gt)

for idx in train_idx:
    gt_datadict = pickle.load(open(os.path.join(res_dir, 'influence_{}_temp0.pkl'.format(idx))))
    hingeinf_datadict = pickle.load(open(os.path.join(sh_dir, 'influence_{}_temp0.0005.pkl'.format(idx))))
    load_data(gt_datadict, hingeinf_datadict, influence_list, hinge_list, gt_list, idx)
...
for fname in file_list:
    ...
    if idx not in train_idx:
        # if 'temp0.0005' not in fname:
        #     continue
        if not os.path.exists(os.path.join(sh_dir, 'influence_{}_temp0.0005.pkl'.format(idx))):
            continue
        gt_datadict = pickle.load(open(os.path.join(res_dir, 'influence_{}_temp0.pkl'.format(idx))))
        hingeinf_datadict = pickle.load(open(os.path.join(sh_dir, 'influence_{}_temp0.0005.pkl'.format(idx))))
        load_data(gt_datadict, hingeinf_datadict, influence_list, hinge_list, gt_list, idx)
...
#     datadict = pickle.load(open(os.path.join(minmax_res_dir, 'influence_{}_temp0.pkl'.format(idx))))
...
#     datadict = pickle.load(open(os.path.join(minmin_res_dir, 'influence_{}_temp0.pkl'.format(idx))))
